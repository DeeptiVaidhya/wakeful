...
abstract class REST_Controller extends CI_Controller {
    ...
    public function check_token($check_token_only = false) {
        ...
        if($check_token_only){
            ...
            $this->auth->count_consecutive_day($user_id);
            $course_id = $this->auth->user_has_course($user_id);
            $study_id = user_has_study($user_id);
            $where = array();
            if(isset($param['study_id']) && $param['study_id'] != null){
                $where = array('where'=> array('study_id'=> $param['study_id']));
            }
            $settings = $this->auth->get_setting($where);

            $this->response(array('success'=>true,'msg'=>'Valid Token','settings'=>$settings, 'course_id' => $course_id, 'study_id' => $study_id), REST_Controller::HTTP_OK);
        }
    }
