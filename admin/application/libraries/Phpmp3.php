...
class PHPMP3
{
	...
	public function save($path) {
		...
		// Load config file
		$CI->load->config('wakeful');
		$config=$CI->config->item('sftp_details');
		...
		$fp = fopen('ssh2.sftp://'.intval($sftp).'/'.$path, 'w');
	...
	public function multiJoin($newpath, $array) {
		...
		$CI->load->config('wakeful');
		$config=$CI->config->item('sftp_details');
		$audio_config = $CI->config->item('sftp_assets_audios');
		$connection = ssh2_connect($config['hostname'], 22);
		ssh2_auth_password($connection, $config['username'], $config['password']);
		$sftp = ssh2_sftp($connection);
		foreach ($array as $path) {
			$mp3 = new static('ssh2.sftp://'.intval($sftp).'/'.$audio_config['path'].basename($path));
			$mp3->striptags();
			$mp3_1 = new static('ssh2.sftp://'.intval($sftp).'/'.$newpath);
			$mp3->mergeBehind($mp3_1);
			$mp3->multi_save($newpath, intval($sftp));
		}
	}

	/**
	 * Common function to Join multiple mp3 files into one file
	 *
	 * @param string $newpath
	 * @param array $array
	 * @return void
	 */
	public function multi_save($path, $sftp_val) {
		$fp = fopen('ssh2.sftp://'.$sftp_val.'/'.$path, 'w+');
		$bytesWritten = fwrite($fp, $this->str);
		fclose($fp);
		return $bytesWritten == strlen($this->str);
	}
